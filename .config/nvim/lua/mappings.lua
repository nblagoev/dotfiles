...
local subs_word = [[:%s/\<<C-r><C-w>\>/<C-r><C-w>/gI<Left><Left><Left>]]
local subs_selection = [[y:%s/<C-r>0/<C-r>0/gI<Left><Left><Left>]]
map_key("v", ",R",       ":s///g<LEFT><LEFT>",                   "Replace search") -- on selection only
map_key("n", ",R",       ":%s///g<LEFT><LEFT>",                  "Replace search") -- on entire buffer
map_key("n", ",X",       subs_word,                              "Substitute current word")
map_key("v", ",X",       subs_selection,                         "Substitute selection")
